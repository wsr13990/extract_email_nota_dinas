...
class OutlookLib:
    ...
    def get_messages(self, folder=None):
        ...
        # help(messages.GetLast)
        # last_n_day = dt.datetime.now() - dt.timedelta(days=n)
        # last_n_day = last_n_day.strftime('%m/%d/%Y')
        # restriction = "[ReceivedTime] >= '" + last_n_day + "'"
        # messages = messages.Restrict(restriction)
        # print(messages.GetFirst())
        return messages
...
def Main():
    ...
    outlook = OutlookLib()
    messages = outlook.get_messages(FOLDER)

    dir_path = os.path.dirname(os.path.realpath(__file__))
    # Loop all messages
    msg = messages.GetFirst()
    for _ in range(NUM_PREV_EMAIL):
        if msg == None:
            print('Failed to read email due to unknown error')
        else:
            # while msg:
            ...
            if len(rekap[rekap['Nota Dinas'].str.strip() == nodin]) >= 1:
                ...
                if os.path.isdir(dest_path):
                    ...
                    for attachTuple in attach:
                        print("Downloading attachments under " +
                              attachTuple[1])
                        for fileAtt in attachTuple[0]:
                            print(dest_path+'/'+fileAtt.FileName)
                            fileAtt.SaveAsFile(
                                dest_path+'/' + fileAtt.FileName)
            ...
            rekap['Folder'] = rekap['Nota Dinas'].map(
                lambda x: x.replace('.', ''))
            rekap['Folder'] = rekap['Folder'].map(lambda x: x.replace('/', ''))
            rekap[['Folder', 'Nota Dinas', 'Title / Subject', 'Tanggal Terima']
                  ].to_csv(TXT_FILE, sep='|', index=False, header=True)
