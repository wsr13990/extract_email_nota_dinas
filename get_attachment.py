...
def get_subject(eml_file):
    with open(eml_file, 'rb') as fp:
        msg = BytesParser(policy=policy.default).parse(fp)
        return msg.get_all('Subject')[0]

def get_nota_dinas(eml_file):
    return get_subject(eml_file).split("(")[-1].split(")")[-2]

def download_attachment(eml_file,dest_path,move_eml=True):
    is_success = False
...
def main():
    ...
    for eml_file in files:
        if eml_file.endswith('.eml'):
            subject = get_subject(eml_file)
            if len(rekap[rekap['Title / Subject'].str.strip()==subject]) >= 1:
#                print('file already processed')
                os.remove(eml_file)
            else:         
                num = max(rekap['No'], default=0) + 1
                folder = get_nota_dinas(eml_file).replace('/','').replace('.','')
                dest_path = os.path.join(dir_path,str(num)+'. '+folder)
                if os.path.isdir(dest_path) == False:
                    os.mkdir(dest_path)
                if os.path.isdir(dest_path):
                    if TEST == True:
                        download_attachment(eml_file,dest_path,False)
                    else:
                        download_attachment(eml_file,dest_path)
                rekap = rekap.append({'No' : num , 'Title / Subject' : subject,
